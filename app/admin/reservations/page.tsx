...
function updateMessage(value?: string) {
  ...
  if (value === "cancelled") return "Reservation cancelled and a cancellation email was sent to the guest.";
  if (value === "email-sent") return "Cancellation email sent to the guest.";
  if (value === "email-failed") return "The reservation was cancelled, but the email could not be delivered. Check email setup and retry from this record.";
  if (value === "cancel-first") return "Cancel this confirmed booking first so the guest receives a cancellation email, then remove the record if needed.";
...
export default async function ReservationsAdminPage({searchParams}: {searchParams: Promise<{update?: string}>}) {
  ...
  const [settings, schedule, reservations, query] = await Promise.all([getBookingSettings(), getRestaurantSchedule(), listReservations(), searchParams]);
  ...
  const calendarExceptions = upcoming.filter((item) => !isWithinSchedule(schedule, item.bookingDate, item.startTime, minutes(item.endTime) - minutes(item.startTime)));
  ...
  return <AdminFrame active="/admin/reservations" session={session}>
    ...
    {query.update && <p className={`adminAlert ${["success", "created", "deleted", "cancelled", "email-sent"].includes(query.update) ? "isSuccess" : "isError"}`}>{updateMessage(query.update)}</p>}
    {calendarExceptions.length > 0 && <p className="adminAlert isError">Review {calendarExceptions.length} confirmed table{calendarExceptions.length === 1 ? "" : "s"} outside the current opening calendar. Honour them as agreed exceptions or contact the guests to rearrange.</p>}
    ...
    <section className="adminPanel"><div className="adminPanelHeading"><div><p>Capacity controls</p><h2>Booking rules</h2></div><b>{settings.bookingEnabled?"Accepting bookings":"Paused"}</b></div>
      <p>Online table availability follows the restaurant calendar. Staff can still record an agreed exception here.</p>
